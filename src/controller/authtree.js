...
layui.define(['jquery', 'form'], function(exports){
	...
	obj = {
		...
		render: function(dst, trees, opt){
			...
			opt.inputname = opt.inputname ? opt.inputname : 'menuids[]';
			opt.layfilter = opt.layfilter ? opt.layfilter : 'checkauth';
			opt.openall = opt.openall ? opt.openall : false;
			$(dst).html(obj.renderAuth(trees, 0, opt));
			form.render();
			...
			$(dst).find('.auth-single:first').unbind('click').on('click', '.layui-form-checkbox', function(){
				...
				if(checked){
					...
					if($(this).parents('.auth-child:first').find('input[type="checkbox"]:checked').length == 0){
						if($(this).parents('.auth-child:first').prev('.auth-status').find('.layui-form-checkbox:first').length==1){
							$(this).parents('.auth-child:first').prev('.auth-status').find('.layui-form-checkbox:first').click();
						}
					}
				}
				/*console.log(childs);*/
		...
		renderAuth: function(tree, dept, opt){
			...
			layui.each(tree, function(index, item){
				...
				str += '<div><div class="auth-status"> '+(hasChild?'<i class="layui-icon auth-icon '+(openall?'active':'')+'" style="cursor:pointer;">'+(openall?'&#xe625;':'&#xe623;')+'</i>':'<i class="layui-icon auth-leaf" style="opacity:0;">&#xe626;</i>')+(dept > 0 ? '<span>├─ </span>':'')+'<input type="checkbox" id="treeInputValue'+item[opt.nodeValue]+'" name="'+inputname+'" title="'+item[opt.nodeName]+'" value="'+item[opt.nodeValue]+'" lay-skin="primary" lay-filter="'+layfilter+'" '+(item.checked?'checked="checked"':'')+'> </div> <div class="auth-child" style="'+(openall?'':'display:none;')+'padding-left:40px;"> '+append+'</div></div>'
			});
			str += '</div>';
			return str;
		},
		...
		setValue: function(dst,valueArray){
			layui.each(valueArray, function(index, item){
				var checkbox = $(dst).find('#treeInputValue'+item);
...
// 					(dept > 0 ? '<span>├─ </span>':'')+
// 					'<input type="checkbox" name="'+inputname+'" title="'+item.title+'" value="'+item.menuId+'" lay-skin="primary" lay-filter="'+layfilter+'" '+
